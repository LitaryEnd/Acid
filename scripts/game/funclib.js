...
const drawFunction = new Packages.arc.struct.Array();
const drawAnimation = new Packages.arc.struct.Array();

const Vars.effect.status = new statusEffectTwo(lifetime, damage, speed, reloadMultiplier, speedMultiplier);

		const new function = new cFunction(statusEffect, Core.effect);
			
// function library made by Eye of darkness, all you will see here is a modification of the original script. 
...
module.exports = {
 ...
 newNavalEffect (lifetime, staticLife, renderer) {
   return new NavalEffectEntity.NavalEffect (lifetime, staticLife, new Effects.EffectRenderer({render:renderer}));
}, 
 newAirEffect (lifetime, staticLife, renderer) {
   return new AirEffectEntity.AirEffect (lifetime, staticLife, new Effects.EffectRenderer({render:renderer}));
}, 
	...
	collideLineHealth(bullet, x, y, angle, length, health, effectInsulated, status){
			...
			if(dst2 < range * range && (result == null || dst2 < cdist) && team != b.getTeam()){
				if(!Vars.android){
					...
					result = new TargetTrait({
						...
						setY: function(y){b.y = y},
						velocity: function(){return b.velocity()},
						//override defaults/переопределение по умолчанию 
	...
	lineTentacleRenderer(x, y, rotation, stroke, strokeOffset, segments, length, startingLength, moveScale, moveMag, moveMagOffset, timingOffset, timeOffset) {
		...
		const segDegreeOffset = moveMagOffset;
		const segFrequency = moveScale;
       const segRotation = rotateSpeed;
       const sedSpeed = moveSpeed;

       const new lineTentacleRenderer => lineTentacle (segments, segmentSpeed);
			
       const segSpeed = segmentSpeed;
		const statusEffectTwo => moveSpeed(lineTentacle);
